...
export default function Home() {
  ...
  const handleCompare = () => {
    ...
    for (let i = 0; i < maxLength; i++) {
      ...
      diff.forEach((part) => {
        if (part.added) {
          highlightedSentence2.push(
            `<span class="bg-red-600 text-white">${part.value}</span>`
          );
        } else if (part.removed) {
          highlightedSentence1.push(
            `<span class="bg-red-600 text-white">${part.value}</span>`
          );
        } else {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 p-4">
      <div className="w-full flex justify-between items-center mb-6">
        <img
          ...
          className="h-12 w-12"
        />
        <h1 className="text-2xl font-bold text-black">Text Compare</h1>
        <div className="h-12 w-12"></div> {/* Placeholder to keep the title centered */}
      </div>
